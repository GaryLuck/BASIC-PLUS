...
7020	Q%=0%\ Q0%=SPDA%\ GOSUB 12200\ Q%=1%\ Q0%=SPTA%\ GOSUB 12200 &
	\ SIZE(0%,0%)=SIZE(0%,0%)+512%+FNW0%(SP%+26%)-SP% &
	\ SIZE(0%,2%)=SIZE(0%,2%)+FNW0%(SP%+26%) &
	\ SIZE(2%,Q%)=SIZE(0%,Q%)+SIZE(1%,Q%) FOR Q%=0% TO 3% &
	\ PRINT #3%\ PRINT #3%, "K-Words Reserved      Used      Free" &
	\ FOR Q%=0% TO 2% &
	  \ Q1=(SIZE(Q%,0%)-SIZE(Q%,1%))/2048.0 &
	  \ Q2=(SIZE(Q%,2%)-SIZE(Q%,3%))/2048.0 &
...
12210	SIZE(Q%,Q1%)=FNW0%(Q0%+Q1%+Q1%+6%) FOR Q1%=0% TO 3% &
	\ RETURN &
